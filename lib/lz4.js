...
class LZ4Wasm {
  ...
  CopyAndFree (ptr, size) {
    const slice = this.U8(ptr, size).slice()
  ...
  DecompressWith (buffer, transform) {
    ...
    if (x === 0) throw new Error('lz4: failed to decompress')
  ...
  DecompressRawWith (size, buffer, transform) {
    ...
    if (x === 0) throw new Error('lz4: failed to decompress (raw)')
