...
class JsonSerializer {
    constructor(opts = {}) {
        ...
        this.$defaultPacker = new MsgPackr.Packr({
            mapsAsObjects: true
        })
    ...
    $_getStructures() {
        ...
        if (Fs.existsSync(path)) {
            ...
            return this.deserializeTo(MsgPackr, rawBuffer)
        } else {
            return []
        }
    }
    $_saveStructures(structures) {
        const path = Utility.getCacheStoragePath(this.$namespace, 'SharedStructures.data')
        const result = this.serializeTo(MsgPackr, structures)
     ...
     */
    serialize(json) {
        return this.serializeTo(MsgPackr, json)
    }
    serializeHC(json) {
        return this.serializeTo(MsgPackr, json, true)
    }
    serializeSimple(json, highComp = false) {
        const packedBuffer = MsgPackr.pack(json)
    ...
    serializeTo(packer = this.$defaultPacker, json, highComp = false) {
    ...
    deserialize(buffer) {
        return this.deserializeTo(MsgPackr, buffer)
    }
    deserializeSimple(buffer) {
        const decompressedBuffer = LZ4.decode(buffer)
        return MsgPackr.unpack(decompressedBuffer)
    }
