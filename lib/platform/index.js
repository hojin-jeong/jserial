...
class BlockFrameStream {
  ...
  #_processBlocks() {
    while (this.#recvLen >= 8) {
      ...
      // Allocate WASM memory for compression buffer
      const lz4 = this.#lz4
      const srcPtr = lz4.Alloc(compressedLen)
      const dstPtr = lz4.Alloc(decompressedLen)

      // Check for allocation failures
      if (!srcPtr || !dstPtr) {
        if (srcPtr) lz4.Free(srcPtr, compressedLen)
        if (dstPtr) lz4.Free(dstPtr, decompressedLen)
      ...
      lz4.Free(srcPtr, compressedLen)

      if (written !== decompressedLen) {
        lz4.Free(dstPtr, decompressedLen)
        this.#_consumeChunks(totalBlockSize)
        continue
      }
  ...
  #_consumeChunks(bytes) {
    let remaining = bytes
    while (remaining > 0 && this.#recvChunks.length > 0) {
      const first = this.#recvChunks[0]
      if (first.length <= remaining) {
        remaining -= first.length
        this.#recvChunks.shift()
      } else {
        this.#recvChunks[0] = first.subarray(remaining)
        remaining = 0
      }
    }
    this.#recvLen -= bytes
